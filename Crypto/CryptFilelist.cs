...
internal static class CryptFilelist
{
    public static void ProcessFilelist(CryptActions cryptAction, string inFile)
    {
        using var inFileReader = new BinaryReader(File.Open(inFile, FileMode.Open, FileAccess.Read), GlobalConfig.DefaultEncoding);
        ...
        const uint readPos = 32;
        const uint writePos = 32;
